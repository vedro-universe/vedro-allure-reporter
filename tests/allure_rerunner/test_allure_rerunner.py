...
__all__ = ("allure_rerunner", "scheduler_", "dispatcher", "sleep_")  # fixtures


@pytest.mark.usefixtures(allure_rerunner.__name__)
async def test_reruns_validation(dispatcher: Dispatcher):
    ...
    with then:
        ...
        assert str(exc_info.value) == "--allure-reruns must be >= 0"
...
async def test_reruns_delay_validation(dispatcher: Dispatcher):
    ...
    with then:
        assert exc_info.type is ValueError
        assert str(exc_info.value) == "--allure-reruns-delay must be >= 0.0"
...
async def test_reruns_delay_without_reruns_validation(dispatcher: Dispatcher):
    ...
    with then:
        assert exc_info.type is ValueError
        assert str(exc_info.value) == "--allure-reruns-delay must be used with --allure-reruns > 0"
